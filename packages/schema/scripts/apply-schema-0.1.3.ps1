...
$statusPath = Join-Path $statusDirectory 'forge-schema-0.1.3-migration.json'
...
try {
    Write-Host ''
    Write-Host 'FORGE: applying Schema 0.1.3 with checksum tracking.' -ForegroundColor Cyan
    ...
    Set-MigrationStatus -Status 'PASS' -Detail 'Schema 0.1.3 migration applied with checksum tracking.'
    Write-Host ''
    Write-Host 'PASS: Schema 0.1.3 applied.' -ForegroundColor Green
    $exitCode = 0
}
catch {
