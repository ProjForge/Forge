...
function Set-Fixture([int64]$FailedCount,[int64]$EFree) {
    [ordered]@{
        ...
        database=[ordered]@{archiveMode='on';archiveTimeout='1h';currentLsn='0/1000000';failedCount=$FailedCount;lastArchivedWal='000000010000000000000001';lastFailedWal=$null}
...
try {
    ...
    $status = Get-Content -LiteralPath (Join-Path $pitr 'status\pitr-monitor.json') -Raw | ConvertFrom-Json
    if ($status.status -ne 'PASS') { throw 'Activated healthy monitor did not pass.' }
