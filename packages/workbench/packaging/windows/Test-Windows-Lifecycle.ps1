...
try {
    ...
    New-TestDistribution $v1 '0.2.0-rc.2' 'payload-v1'
    New-TestDistribution $v2 '0.2.0-rc.3' 'payload-v2'
    ...
    if ((Get-Content -Raw -LiteralPath (Join-Path $installRoot 'RELEASE.json') | ConvertFrom-Json).version -ne '0.2.0-rc.3') { throw 'Installed release metadata was not updated.' }
